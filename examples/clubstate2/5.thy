...
\<and> ((length waiting) < 4 \<longrightarrow> ((card onCourt' = 2)
 \<or> (card onCourt' = 3)))
...
"ClubState => ClubState =>  STUDENT list \<Rightarrow> STUDENT set
 =>  STUDENT set =>  STUDENT set => STUDENT => bool"
where 
...
"ClubState => ClubState =>  STUDENT set =>  STUDENT set => 
 STUDENT set =>  STUDENT list => bool"
where 
