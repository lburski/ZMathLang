...
\where
    ...
    \expression{\set{birthday'} = \set{\set{birthday} \cup \set{\{\term{\term{name?} \mapsto \term{date?}}\}}}}}
\end{schema}
...
\begin{schema}{Remind}
	\text{\Xi BirthdayBook} \\
	\text{\declaration{\term{today?}: \expression{DATE}} \\
\declaration{\set{cards!}: \expression{\power NAME}}}\\
\where
	\text{\expression{\set{cards!} = \set{\{\declaration{\term{n}: \expression{known}} | \expression{\term{\set{birthday}		(\term{n})} = \term{today?}}\}}}}
\end{schema}
